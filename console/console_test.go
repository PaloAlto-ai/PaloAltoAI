...
func newTester(t *testing.T, confOverride func(*paa.Config)) *tester {
	...
	// Create a networkless protocol stack and start an PaloAltoAi service within
	...
	paaConf := &paa.Config{
		...
		Paaerbase: common.HexToAddress(testAddress),
		Paaash: paaash.Config{
			PowMode: paaash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(paaConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return paa.New(ctx, paaConf) }); err != nil {
		t.Fatalf("failed to register PaloAltoAi protocol: %v", err)
	}
	...
	stack.Service(&PaloAltoAi)

	return &tester{
		workspace: workspace,
		stack:     stack,
		PaloAltoAi:  PaloAltoAi,
		console:   console,
		input:     prompter,
		output:    printer,
	}
