...
import (
	...
	"github.com/PaloAltoAi/go-PaloAltoAi/internal/jsre"
	"github.com/PaloAltoAi/go-PaloAltoAi/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute PaloAltoAi requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jpaa", struct{}{})

	jpaaObj, _ := c.jsre.Get("jpaa")
	jpaaObj.Object().Set("send", bridge.Send)
	jpaaObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jpaa);"); err != nil {
	...
	flatten := "var paa = web3.paa; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = paa.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jpaa.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jpaa.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jpaa.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jpaa.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jpaa.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines paa.getBalance(paa.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gpaa instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gpaa metadata
	fmt.Fprintf(c.printer, "Welcome to the Gpaa JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + paa.coinbase);
		console.log("at block: " + paa.blockNumber + " (" + new Date(1000 * paa.getBlock(paa.blockNumber).timestamp) + ")");
