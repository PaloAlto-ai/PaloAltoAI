...
const (
	...
	URN_WANPaaernetLinkConfig_1    = "urn:schemas-upnp-org:service:WANPaaernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Paaernet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Paaernet
//
...
// WANPaaernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANPaaernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANPaaernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANPaaernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANPaaernetLinkConfig1Clients() (clients []*WANPaaernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANPaaernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANPaaernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANPaaernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANPaaernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANPaaernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANPaaernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANPaaernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANPaaernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANPaaernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANPaaernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANPaaernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANPaaernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANPaaernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANPaaernetLinkConfig1 {
	clients := make([]*WANPaaernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANPaaernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewPaaernetLinkStatus: allowed values: Up, Down
func (client *WANPaaernetLinkConfig1) GetPaaernetLinkStatus() (NewPaaernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewPaaernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANPaaernetLinkConfig_1, "GetPaaernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewPaaernetLinkStatus, err = soap.UnmarshalString(response.NewPaaernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
