...
// Reference: https://github.com/PaloAltoAi/wiki/wiki/Paaash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see paaash_io_prepare
enum paaash_io_rc {
	PAAASH_IO_FAIL = 0,           ///< There has been an IO failure
	PAAASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	PAAASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	PAAASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of paaash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * paaash_full_t
 */
#ifdef PAAASH_PRINT_CRITICAL_OUTPUT
#define PAAASH_CRITICAL(...)							\
	do													\
	{													\
		printf("PAAASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for paaash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the paaash
 ...
 *                           https://github.com/PaloAltoAi/wiki/wiki/Paaash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum paaash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/PaloAltoAi/wiki/wiki/Paaash-DAG
 *
...
static inline bool paaash_io_mutable_name(
	uint32_t revision,
