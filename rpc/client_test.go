	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("PaaSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("PaaSubscribe shouldn't have panicked for %#v", arg)
		...
		client.PaaSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("paa", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.PaaSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while PaaSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("paa", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.PaaSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("PaaSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("PaaSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("PaaSubscribe did not return within 1s after Close")
	}
}

// This test reproduces https://github.com/PaloAltoAi/go-PaloAltoAi/issues/17837 where the
...
func TestClientCloseUnsubscribeRace(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		client := DialInProc(server)
		nc := make(chan int)
		sub, err := client.PaaSubscribe(context.Background(), nc, "someSubscription", 3, 1)
		if err != nil {
			t.Fatal(err)
		}
		go client.Close()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.PaaSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "paa_echo", i)
			if err != nil {
				if !wantError {
...
func TestClientReconnect(t *testing.T) {
	...
	// We spawn multiple calls here to check whpaaer this hangs somehow.
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-PaloAltoAi-test-ipc-%d-%d", os.Getpid(), rand.Int63())
