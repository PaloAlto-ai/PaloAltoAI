...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout paa_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for PaaSubscribe requests
}
...
// PaaSubscribe registers a subscripion under the "paa" namespace.
func (c *Client) PaaSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "paa", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. PaaSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through PaaSubscribe.
