...
import (
	...
	"github.com/PaloAltoAi/go-PaloAltoAi/signer/core"
	"github.com/PaloAltoAi/go-PaloAltoAi/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx paaapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx paaapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(paaapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := paaapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx paaapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
