...
import (
	...
	"github.com/prompaaeus/prompaaeus/util/flock"
)
...
func (n *Node) OpenDatabase(name string, cache, handles int) (paadb.Database, error) {
	if n.config.DataDir == "" {
		return paadb.NewMemDatabase(), nil
	}
	return paadb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
