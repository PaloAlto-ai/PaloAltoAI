...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (paadb.Database, error) {
	if ctx.config.DataDir == "" {
		return paadb.NewMemDatabase(), nil
	}
	db, err := paadb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
