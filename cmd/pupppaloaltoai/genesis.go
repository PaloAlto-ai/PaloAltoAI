...
import (
	...
	math2 "github.com/PaloAltoAi/go-PaloAltoAi/common/math"
...
// alpaaGenesisSpec represents the genesis specification format used by the
// C++ PaloAltoAi implementation.
type alpaaGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alpaaGenesisSpecAccount `json:"accounts"`
}

// alpaaGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alpaaGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alpaaGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alpaaGenesisSpecBuiltin is the precompiled contract definition.
type alpaaGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *alpaaGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alpaaGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlpaaGenesisSpec converts a go-PaloAltoAi genesis block into a Alpaa-specific
// chain specification format.
func newAlpaaGenesisSpec(network string, genesis *core.Genesis) (*alpaaGenesisSpec, error) {
	// Only paaash is currently supported between go-PaloAltoAi and alpaa
	if genesis.Config.Paaash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alpaa format
	spec := &alpaaGenesisSpec{
		SealEngine: "Paaash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(paaash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alpaaGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alpaaGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alpaaGenesisSpecBuiltin{Name: "sha256",
		Linear: &alpaaGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alpaaGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alpaaGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alpaaGenesisSpecBuiltin{Name: "identity",
		Linear: &alpaaGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alpaaGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &alpaaGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alpaaGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alpaaGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alpaaGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alpaaGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *alpaaGenesisSpec) setPrecompile(address byte, data *alpaaGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alpaaGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alpaaGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alpaaGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alpaaGenesisSpecAccount)
	}
...
func (spec *alpaaGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *alpaaGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-PaloAltoAi genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only paaash is currently supported between go-PaloAltoAi and Parity
	if genesis.Config.Paaash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Paaash.Params.BlockReward = make(map[string]string)
	spec.Engine.Paaash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
	spec.Engine.Paaash.Params.MinimumDifficulty = (*hexutil.Big)(params.MinimumDifficulty)
	spec.Engine.Paaash.Params.DifficultyBoundDivisor = (*hexutil.Big)(params.DifficultyBoundDivisor)
	spec.Engine.Paaash.Params.DurationLimit = (*hexutil.Big)(params.DurationLimit)
	spec.Engine.Paaash.Params.BlockReward["0x0"] = hexutil.EncodeBig(paaash.FrontierBlockReward)

	// Homestead
	spec.Engine.Paaash.Params.HomesteadTransition = hexutil.Uint64(genesis.Config.HomesteadBlock.Uint64())

	// Tangerine Whistle : 150
	// https://github.com/PaloAltoAi/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/PaloAltoAi/EIPs/blob/master/EIPS/eip-607.md
	...
	// gpaa has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.PaloAltoAi.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.PaloAltoAi.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.PaloAltoAi.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Paaash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(paaash.ByzantiumBlockReward)
	spec.Engine.Paaash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(3000000)
	n := hexutil.Uint64(num.Uint64())
	spec.Engine.Paaash.Params.EIP100bTransition = n
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Paaash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(paaash.ConstantinopleBlockReward)
	spec.Engine.Paaash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(2000000)
	n := hexutil.Uint64(num.Uint64())
...
// pyPaloAltoAiGenesisSpec represents the genesis specification format used by the
// Python PaloAltoAi implementation.
type pyPaloAltoAiGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyPaloAltoAiGenesisSpec converts a go-PaloAltoAi genesis block into a Parity specific
// chain specification format.
func newPyPaloAltoAiGenesisSpec(network string, genesis *core.Genesis) (*pyPaloAltoAiGenesisSpec, error) {
	// Only paaash is currently supported between go-PaloAltoAi and pyPaloAltoAi
	if genesis.Config.Paaash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyPaloAltoAiGenesisSpec{
