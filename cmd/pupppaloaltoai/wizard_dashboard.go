...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"paastats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "paastats":
				if infos, err := checkPaastats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have paastats running, ask whpaaer to make the secret public or not
	if w.conf.paastats != "" {
		fmt.Println()
		fmt.Println("Include paastats secret on dashboard (y/n)? (default = yes)")
