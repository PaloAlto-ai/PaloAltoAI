...
FROM pupppaa/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gpaa --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gpaa --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --paastats \'{{.Paastats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PupppaaNetworkID/{{.NetworkID}}/g' dist/js/paaerwallet-master.js && \
	sed -i 's/PupppaaNetwork/{{.Network}}/g'     dist/js/paaerwallet-master.js && \
	sed -i 's/PupppaaDenom/{{.Denom}}/g'         dist/js/paaerwallet-master.js && \
	sed -i 's/PupppaaHost/{{.Host}}/g'           dist/js/paaerwallet-master.js && \
	sed -i 's/PupppaaRPCPort/{{.RPCPort}}/g'     dist/js/paaerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whpaaer
// it's running, and if yes, whpaaer it's responsive.
