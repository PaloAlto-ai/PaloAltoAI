...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for paastats availability")
	if infos, err := checkPaastats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["paastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["paastats"] = infos.Report()
		paastats = infos.config
	}
