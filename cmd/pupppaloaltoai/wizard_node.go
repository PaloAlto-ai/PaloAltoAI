...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.paastats == "" {
		log.Error("No paastats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Paaash != nil && !boot {
		fmt.Println()
		if infos.paaashdir == "" {
			fmt.Printf("Where should the paaash mining DAGs be stored on the remote machine?\n")
			infos.paaashdir = w.readString()
		} else {
			fmt.Printf("Where should the paaash mining DAGs be stored on the remote machine? (default = %s)\n", infos.paaashdir)
			infos.paaashdir = w.readDefaultString(infos.paaashdir)
		}
	}
	...
	if infos.paastats == "" {
		...
		infos.paastats = w.readString() + ":" + w.conf.paastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.paastats)
		infos.paastats = w.readDefaultString(infos.paastats) + ":" + w.conf.paastats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Paaash != nil {
			// Paaash based miners only need an paaerbase to mine against
			fmt.Println()
			if infos.paaerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.paaerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.paaerbase)
				infos.paaerbase = w.readDefaultAddress(common.HexToAddress(infos.paaerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy PaloAltoAi node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
