...
// nodeDockerfile is the Dockerfile required to run an PaloAltoAi node.
var nodeDockerfile = `
FROM PaloAltoAi/client-go:latest
...
{{end}}
RUN \
  echo 'gpaa --cache 512 init /genesis.json' > gpaa.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.PaloAltoAi/keystore/ && cp /signer.json /root/.PaloAltoAi/keystore/' >> gpaa.sh && \{{end}}
	echo $'exec gpaa --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --paastats \'{{.Paastats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Paaerbase}}--miner.paaerbase {{.Paaerbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gpaa.sh

ENTRYPOINT ["/bin/sh", "gpaa.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an PaloAltoAi node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.PaloAltoAi{{if .Paaashdir}}
      - {{.Paaashdir}}:/root/.paaash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Paastats}}
      - MINER_NAME={{.Paaerbase}}
...
// deployNode deploys a new PaloAltoAi node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.paaerbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Paaashdir":  config.paaashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Paastats":   config.paastats[:strings.Index(config.paastats, ":")],
		"Paaerbase":  config.paaerbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Paastats username":        info.paastats,
	}
	if info.gasTarget > 0 {
		...
		if info.paaerbase != "" {
			// Paaash proof-of-work miner
			report["Paaash directory"] = info.paaashdir
			report["Miner account"] = info.paaerbase
		}
		if info.keyJSON != "" {
...
// whpaaer it's running, and if yes, whpaaer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gpaa --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.PaloAltoAi"],
		paaashdir:  infos.volumes["/root/.paaash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		paastats:   infos.envvars["STATS_NAME"],
		paaerbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
