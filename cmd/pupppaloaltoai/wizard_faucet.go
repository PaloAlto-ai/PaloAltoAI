...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Paaers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.paastats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.paastats = w.readString() + ":" + w.conf.paastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.paastats)
		infos.node.paastats = w.readDefaultString(infos.node.paastats) + ":" + w.conf.paastats
	}
