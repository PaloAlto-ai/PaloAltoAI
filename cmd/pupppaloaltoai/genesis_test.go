...
// Tests the go-PaloAltoAi to Alpaa chainspec conversion for the Stureby testnet.
func TestAlpaaSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gpaa.json")
	if err != nil {
	...
	spec, err := newAlpaaGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alpaa.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alpaaGenesisSpec{}
...
// Tests the go-PaloAltoAi to Parity chainspec conversion for the Stureby testnet.
