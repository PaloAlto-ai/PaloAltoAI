...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Paaash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of paaash, we're pretty much done
		genesis.Config.Paaash = new(params.PaaashConfig)
...
// importGenesis imports a Gpaa genesis spec into pupppaa.
...
func (w *wizard) manageGenesis() {
	// Figure out whpaaer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alpaa.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by pupppaa and Gpaa
		gpaaJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gpaaJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gpaaJson)

		// Export the genesis spec used by Alpaa (formerly C++ PaloAltoAi)
		if spec, err := newAlpaaGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alpaa chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alpaa", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly PaloAltoAiJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not sompaaing I can do")
		return
	}
}
