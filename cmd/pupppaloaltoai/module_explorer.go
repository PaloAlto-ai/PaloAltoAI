...
FROM pupppaa/explorer:latest

ADD paastats.json /paastats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../paa-net-intelligence-api && pm2 start /paastats.json)' >  explorer.sh && \
	echo '(cd ../paaerchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerPaastats is the configuration file for the paastats javascript client.
var explorerPaastats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.PaloAltoAi
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Paastats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	paastats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerPaastats)).Execute(paastats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.paastats[:strings.Index(config.paastats, ":")],
		"Secret": config.paastats[strings.Index(config.paastats, ":")+1 : strings.Index(config.paastats, "@")],
		"Host":   config.paastats[strings.Index(config.paastats, "@")+1:],
	})
	files[filepath.Join(workdir, "paastats.json")] = paastats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.PaloAltoAi"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		paastats: infos.envvars["STATS"],
	}
	return stats, nil
}
