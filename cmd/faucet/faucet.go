...
// faucet is a Paaer faucet backed by a light client.
package main
...
var (
	...
	paaPortFlag = flag.Int("paaport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the PaloAltoAi protocol")
	statsFlag   = flag.String("paastats", "", "Paastats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Paaers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for PaloAltoAi and the faucet")
)

var (
	paaer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Paaers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *paaPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // PaloAltoAi address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an PaloAltoAi light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // PaloAltoAi protocol stack
	client *paaclient.Client   // Client connection to the PaloAltoAi chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the PaloAltoAi light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := paa.DefaultConfig
	...
	// Assemble the paastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightPaloAltoAi
			ctx.Service(&serv)
			return paastats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := paaclient.NewClient(api)
...
// close terminates the PaloAltoAi connection and tears down the faucet.
...
// apiHandler handles requests for Paaer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, paaer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the PaloAltoAi address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Sompaaing funky happened, please open an issue at https://github.com/PaloAltoAi/go-PaloAltoAi/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), paaer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, paaer)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and PaloAltoAi address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the PaloAltoAi address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No PaloAltoAi address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and PaloAltoAi address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain PaloAltoAi address,
