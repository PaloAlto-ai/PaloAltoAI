...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.BoolFlag{
			Name:        "single",
			Usage:       "whpaaer to fetch content from a single node or from all nodes",
			Destination: &single,
		},
		cli.IntFlag{
			Name:        "track-timeout",
...
func emitMetrics(ctx *cli.Context) error {
	if gpaametrics.Enabled {
		...
		return influxdb.InfluxDBWithTagsOnce(gpaametrics.DefaultRegistry, endpoint, database, username, password, "swarm-smoke.", tagsMap)
	}
