...
import (
	...
	"github.com/PaloAltoAi/go-PaloAltoAi/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whpaaer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whpaaer the Trezor is opened, closed
// or whpaaer the PaloAltoAi app was not started on it.
...
// PaloAltoAi address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.PaloAltoAiAddress)
	if _, err := w.trezorExchange(&trezor.PaloAltoAiGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.PaloAltoAiSignTx{
	...
	response := new(trezor.PaloAltoAiTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.PaloAltoAiTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
