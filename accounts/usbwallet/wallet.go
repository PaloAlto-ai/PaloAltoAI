...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the PaloAltoAi
...
type wallet struct {
	...
	deriveChain    PaloAltoAi.ChainStateReader // Blockchain state reader to discover used account with
...
// whpaaer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived PaloAltoAi account
			...
			// If the next account is empty, stop self-derivation, but add it nonpaaeless
...
// Contains implements accounts.Wallet, returning whpaaer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain PaloAltoAi.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
