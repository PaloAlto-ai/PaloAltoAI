...
func NewLesServer(paa *paa.PaloAltoAi, config *paa.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(paa.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, paa.EventMux(), paa.Engine(), newPeerSet(), paa.BlockChain(), paa.TxPool(), paa.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(paa.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(paa.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(paa.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(paa.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(paa.ChainDb())
	return srv, nil
}
...
func newCostStats(db paadb.Database) *requestCostStats {
