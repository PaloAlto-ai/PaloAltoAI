...
func newFreeClientPool(db paadb.Database, connectedLimit, totalLimit int, clock mclock.Clock) *freeClientPool {
	pool := &freeClientPool{
		db:             db,
		clock:          clock,
...
func (f *freeClientPool) connect(address string, disconnectFn func()) bool {
	...
	// check whpaaer (linUsage+connectedBias) is smaller than the highest entry in the connected pool
