...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid paaash solution.
	staleThreshold = 7
)
...
func (paaash *Paaash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if paaash.config.PowMode == ModeFake || paaash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", paaash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if paaash.shared != nil {
		return paaash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	paaash.lock.Lock()
	threads := paaash.threads
	if paaash.rand == nil {
		...
		paaash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	paaash.lock.Unlock()
	if threads == 0 {
	...
	if paaash.workCh != nil {
		paaash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			paaash.mine(block, id, nonce, abort, locals)
		}(i, uint64(paaash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", paaash.SealHash(block.Header()))
			}
			close(abort)
		case <-paaash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := paaash.Seal(chain, block, results, stop); err != nil {
...
func (paaash *Paaash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = paaash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = paaash.dataset(number, false)
	)
	...
	logger.Trace("Started paaash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
			// Mining terminated, update stats and abort
			logger.Trace("Paaash nonce search aborted", "attempts", nonce-seed)
			paaash.hashrate.Mark(attempts)
			break search
		...
		default:
			...
			if new(big.Int).SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("Paaash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("Paaash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func (paaash *Paaash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := paaash.SealHash(block.Header())
	...
	// whpaaer the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := paaash.verifySeal(nil, header, true); err != nil {
		...
		if results == nil {
			log.Warn("Paaash result channel is empty, submitted mining result is rejected")
			return false
		}
	...
	for {
		...
		case result := <-paaash.submitWorkCh:
		...
		case errc := <-paaash.exitCh:
			// Exit remote loop if paaash is closed and return relevant error.
			errc <- nil
			log.Trace("Paaash remote sealer is exiting")
			return
		}
	}
}
