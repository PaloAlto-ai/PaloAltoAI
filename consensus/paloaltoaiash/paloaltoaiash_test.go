...
// Tests that paaash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	paaash := NewTester(nil, false)
	defer paaash.Close()

	results := make(chan *types.Block)
	err := paaash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := paaash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/PaloAltoAi/go-PaloAltoAi/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "paaash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *Paaash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := paaash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	paaash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := paaash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{paaash}
	if _, err := api.GetWork(); err != errPaaashStopped {
		t.Error("expect to return an error to indicate paaash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped paaash")
	}
}
