...
// Tests whpaaer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom paaash engine
	paaash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer paaash.Close()
	...
	paaash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := paaash.SealHash(header).Hex(); work[0] != want {
...
// Tests whpaaer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	paaash := NewTester(nil, true)
	defer paaash.Close()
	api := &API{paaash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			paaash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, paaash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch paaash result timeout", id+1)
		}
	}
}
