...
creating a collection of devp2p nodes, connecting them togpaaer to form a
...
connecting two nodes togpaaer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of sompaaing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whpaaer or not a
...
As a concrete example, consider a simulated network of PaloAltoAi nodes. An
