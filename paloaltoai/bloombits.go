...
const (
	// bloomServiceThreads is the number of goroutines used globally by an PaloAltoAi
...
func (paa *PaloAltoAi) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-paa.shutdownChan:
					return

				case request := <-paa.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(paa.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(paa.chainDb, task.Bit, section, head); err == nil {
...
// for the PaloAltoAi header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      paadb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db paadb.Database, size, confirms uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := paadb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
