...
// stateReq represents a batch of state fetch requests grouped togpaaer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whpaaer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting sompaaing else, ignore the stale timeout.
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for sompaaing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < paadb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
// delivered. Returns whpaaer the peer actually managed to deliver anything of
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for hash, task := range req.tasks {
		// If the node did deliver sompaaing, missing items may be due to a protocol
...
// peer into the state trie, returning whpaaer anything useful was written or any
// error occurred.
