...
import (
	...
	"github.com/PaloAltoAi/go-PaloAltoAi/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gpaa console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = paa.getBlock(paa.getTransaction(tx).blockHash);
  var genesis = paa.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.paa.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  paa.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	...
	*/
	...
	statedb := tests.MakePreState(paadb.NewMemDatabase(), alloc)
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(paadb.NewMemDatabase(), test.Genesis.Alloc)
