...
// This file contains a miner stress test based on the Paaash consensus engine.
package main
...
func main() {
	...
	// Pre-generate the paaash mining DAG so we don't race
	paaash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".paaash"))

	// Create an Paaash network based off of the Ropsten config
	...
	for _, node := range nodes {
		...
		if err := node.Service(&PaloAltoAi); err != nil {
			panic(err)
		}
		if err := PaloAltoAi.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var PaloAltoAi *paa.PaloAltoAi
		if err := nodes[index%len(nodes)].Service(&PaloAltoAi); err != nil {
			panic(err)
		}
		...
		if err := PaloAltoAi.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := PaloAltoAi.TxPool().Stats(); pend > 2048 {
			time.Sleep(100 * time.Millisecond)
		}
	}
}

// makeGenesis creates a custom Paaash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the PaloAltoAi node
	...
	// Start the node and configure a full PaloAltoAi node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return paa.New(ctx, &paa.Config{
			Genesis:         genesis,
			...
			GPO:             paa.DefaultConfig.GPO,
			Paaash:          paa.DefaultConfig.Paaash,
