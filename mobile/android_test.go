...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gpaa.LightScryptN, Gpaa.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.PaloAltoAi", new NodeConfig());
			node.start();
			...
			PaloAltoAiClient ec = node.getPaloAltoAiClient();
	...
	// signatures too. Regression test for go-PaloAltoAi issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogpaaer
	...
	workspace, err := ioutil.TempDir("", "gpaa-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/PaloAltoAi/gpaatest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gpaa and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.PaloAltoAi", "github.com/PaloAltoAi/go-PaloAltoAi/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gpaa.aar"), "gpaa.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "PaloAltoAi", "gpaatest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.PaloAltoAi.gpaatest"
