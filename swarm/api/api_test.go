...
import (
	...
	"github.com/PaloAltoAi/go-PaloAltoAi/swarm/sctx"
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.paa"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.paa"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.paa"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	paaAddr := "swarm.paa"
	paaHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	paaResolve := newTestResolveValidator(paaHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(paaResolve, "")),
			addr:   paaAddr,
			result: paaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(paaResolve, ""),
				MultiResolverOptionWithResolver(paaResolve, ""),
			),
			addr:   paaAddr,
			result: paaHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(paaResolve, "paa"),
			),
			addr:   paaAddr,
			result: paaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "paa"),
				MultiResolverOptionWithResolver(doesntResolve, "paa"),
				MultiResolverOptionWithResolver(paaResolve, "paa"),
			),
			addr:   paaAddr,
		...
		},
		{
			...
			addr:   paaAddr,
			result: paaHash,
			err:    errors.New(`DNS name not found: "swarm.paa"`),
		},
		{
