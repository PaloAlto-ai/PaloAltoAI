...
import (
	...
	swarmhttp "github.com/PaloAltoAi/go-PaloAltoAi/swarm/api/http"
...
func TestClientBzzWithFeed(t *testing.T) {
	...
	// put togpaaer some data for our test:
	dataBytes := []byte(`
	//
	...
	if len(manifestAddress) != storage.AddressLength {
		t.Fatalf("Sompaaing went wrong. Got a hash of an unexpected length. Expected %d bytes. Got %d", storage.AddressLength, len(manifestAddress))
	}
