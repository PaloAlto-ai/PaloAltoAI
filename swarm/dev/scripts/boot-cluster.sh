...
#
# The cluster contains a bootnode, a gpaa node and multiple swarm nodes, with
...
# static gpaa configuration
GPAA_IP="192.168.33.3"
GPAA_RPC_PORT="8545"
GPAA_RPC_URL="http://${GPAA_IP}:${GPAA_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gpaa and
# swarm nodes
start_bootnode() {
...
# start_gpaa_node starts a gpaa node with --datadir pointing at <base-dir>/gpaa
# and a single, unlocked account with password "gpaa"
start_gpaa_node() {
  local dir="${base_dir}/gpaa"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gpaa account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GPAA_IP}"
    --rpcport   "${GPAA_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gpaa" "${GPAA_IP}" "$(which gpaa)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vpaa pair
  local vpaa0="vpaa${name}0"
  local vpaa1="vpaa${name}1"
  ip link add name "${vpaa0}" type vpaa peer name "${vpaa1}"

  # add one end to the bridge
  ip link set dev "${vpaa0}" master "${BRIDGE_NAME}"
  ip link set dev "${vpaa0}" up

  # add the other end to the namespace, rename it paa0 and give it the ip
  ip link set dev "${vpaa1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vpaa1}" name "paa0"
  ip netns exec "${name}" ip link set dev "paa0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "paa0"
}
...
create_account() {
  ...
  gpaa --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
