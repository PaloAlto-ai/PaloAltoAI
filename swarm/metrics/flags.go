...
import (
	...
	gpaametrics "github.com/PaloAltoAi/go-PaloAltoAi/metrics"
...
func Setup(ctx *cli.Context) {
	if gpaametrics.Enabled {
		...
		go gpaametrics.CollectProcessMetrics(2 * time.Second)
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gpaametrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", tagsMap)
		}

		if enableAccountingExport {
			log.Info("Exporting swarm accounting metrics to InfluxDB")
			go influxdb.InfluxDBWithTags(gpaametrics.AccountingRegistry, 10*time.Second, endpoint, database, username, password, "accounting.", tagsMap)
		}
	}
}
