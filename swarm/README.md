...
[https://swarm.PaloAltoAi.org](https://swarm.PaloAltoAi.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the PaloAltoAi web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/PaloAltoAi/go-PaloAltoAi.svg?branch=master)](https://travis-ci.org/PaloAltoAi/go-PaloAltoAi)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/paaersphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/PaloAltoAi/go-PaloAltoAi

    go install github.com/PaloAltoAi/go-PaloAltoAi/cmd/swarm
...
To run Swarm you need an PaloAltoAi account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the PaloAltoAi address representing that account. For example:
...
### PaloAltoAi Name Service resolution

The PaloAltoAi Name Service is the PaloAltoAi equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.paa`), `swarm` has to connect to a `gpaa` instance, which is synced with the PaloAltoAi mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.PaloAltoAi/gpaa.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/PaloAltoAi/go-PaloAltoAi`.

Most likely you will be working from your fork of `go-PaloAltoAi`, let's say from `github.com/nirname/go-PaloAltoAi`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-PaloAltoAi.git $GOPATH/src/github.com/PaloAltoAi/go-PaloAltoAi
```
...
```
go test -v -cpu 4 ./paa -run TestMethod
```
...
We have built a tool to help with automatic start of Grafana and InfluxDB and provisioning of dashboards at https://github.com/nonsense/statpaa , which requires that you have Docker installed.

Once you have `statpaa` installed, and you have Docker running locally, you have to:

1. Run `statpaa` and keep it running in the background
```
statpaa --rm --grafana-dashboards-folder $GOPATH/src/github.com/PaloAltoAi/go-PaloAltoAi/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The PaloAltoAi Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.paa

Their source code can be found at: https://github.com/paaersphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/paaersphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/PaloAltoAi/go-PaloAltoAi/wiki/Code-Review-Guidelines).
