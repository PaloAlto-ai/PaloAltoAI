...
type node struct {
	isLeft      bool      // whpaaer it is left side of the parent double segment
...
type AsyncHasher struct {
	*Hasher            // extends the Hasher
	...
	double  bool       // whpaaer to use double segments (call Hasher.writeSection)
...
// meta: metadata to hash togpaaer with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash togpaaer meta and BMT root hash using the pools
