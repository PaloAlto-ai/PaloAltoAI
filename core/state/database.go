...
// Trie is a PaloAltoAi Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb paadb.Putter) error
}
...
func NewDatabase(db paadb.Database) Database {
...
func NewDatabaseWithCache(db paadb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb paadb.Putter) error {
