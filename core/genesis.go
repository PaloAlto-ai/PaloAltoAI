...
func SetupGenesisBlock(db paadb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	return SetupGenesisBlockWithOverride(db, genesis, nil)
}
func SetupGenesisBlockWithOverride(db paadb.Database, genesis *Genesis, constantinopleOverride *big.Int) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllPaaashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	...
	// Check whpaaer the genesis block is already written.
	if genesis != nil {
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllPaaashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db paadb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db paadb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
	...
	config := g.Config
	if config == nil {
		config = params.AllPaaashProtocolChanges
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db paadb.Database) *types.Block {
...
func GenesisBlockForTesting(db paadb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the PaloAltoAi main net genesis block.
...
// DeveloperGenesisBlock returns the 'gpaa --dev' genesis block. Note, this must
// be seeded with the
