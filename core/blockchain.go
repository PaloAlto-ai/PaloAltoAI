...
// Package core implements the PaloAltoAi consensus protocol.
package core
...
type CacheConfig struct {
	Disabled       bool          // Whpaaer to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     paadb.Database // Low level persistent database to store final content in
	...
	shouldPreserve func(*types.Block) bool // Function used to determine whpaaer should preserve the given block.
}

// NewBlockChain returns a fully initialised block chain using information
// available in the database. It initialises the default PaloAltoAi Validator and
// Processor.
func NewBlockChain(db paadb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= paadb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - paadb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have sompaaing meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
	// Do a sanity check that the provided chain is actually ordered and linked
...
//
// The verify parameter can be used to fine tune whpaaer nonce verification
